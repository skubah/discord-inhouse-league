    ...
    if (userMessage.startsWith('!addrole')) {
        ...
        if (enforceWordCount(userMessage, 2)) {
            ...
            if (selectedRole === 'melee' || selectedRole === 'ranged' || selectedRole === 'support') {
                ...
                const message = `${msg.author.username} has been designated as a ${selectedRole} player.`;
                logger.log('info', `${msg.author.username} has been designated as a ${selectedRole} player.`);
			 ...
			 return {
                    responseMessage: 'Parece que temos um fanfarrão aqui...',
                    // deleteSenderMessage: true
                };
    ...
    if (userMessage.startsWith('!removerole')) {
        ...
        if (enforceWordCount(userMessage, 2)) {
            ...
            if (selectedRole === 'melee' || selectedRole === 'ranged' || selectedRole === 'support') {
                ...
                const message = `${msg.author.username} is no longer a ${selectedRole} player.`;
                logger.log('info', `${msg.author.username} is no longer a ${selectedRole} player.`);
