            ...
            if (isCaptain(msg.author.id)) {
                ...
                if (currentDraft.matchInProgress) {
					...
					if (winOrLoss === 'error') {
                            ...
                            return {
                                responseMessage: 'Error - There was an error.',
                                sendToDm: false
                            };
                    ...
                    if (currentDraft.reportedScores.teamA && currentDraft.reportedScores.teamB) {
                        ...
                        if (
                            ...
                            for (const channel of cmdChannels) {
                                if (currentDraft.league === 'amateur' && channel.name === 'match-bot'
                                        || currentDraft.league === 'pro' && channel.name === 'pro-match-bot') {
                                    await channel.send(endMessage).catch(console.error);
                                }
                            }
                            logger.log('info', `Match reported! Winner is ${currentDraft.reportedScores.teamA}!`);
                            removeDraft(currentDraft.index);
                            ...
                            await recordMatch(
								...
								responseMessage:
                                    `Match reported! Winner is ${currentDraft.reportedScores.teamA}!`;
                        } else {
